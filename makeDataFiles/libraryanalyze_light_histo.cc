...
int main() { 
    ...
    for(int event = 0; event < max_events; event++) {
        ...
        if ( (event % (max_events/10) == 0) && (event != 0) ) {
        ...
        double position[3]; //creates an empty array of length 3
        int rand_voxel;

        // 3 possible cases: random (x,y,z), fixed x & random (y,z) and fixed (x,y,z) - this choice is made in the header file
        if(random_pos == true) { // choose a random voxel and find its co-ords
    ...
    for(int events = 0; events < max_events; events++) {
        ...
        for(int pmt_loop = 0; pmt_loop < 120; pmt_loop++) { //ALTERED FOR DUNE
            ...
            data_z_pos_vuv = pmt_hits.at(4);
            data_z_pos_vis = pmt_hits.at(4);
            ...
            if (do_timings) {
                ...
                double distance_to_pmt = std::sqrt((pmt_hits.at(2)-x_pmt)*(pmt_hits.at(2)-x_pmt) + (pmt_hits.at(3)-y_pmt)*(pmt_hits.at(3)-y_pmt) + (pmt_hits.at(4)-z_pmt)*(pmt_hits.at(4)-z_pmt));
                ...
                for(auto& x: transport_time_vuv) { //we're just looping through the transport_time_vuv vector here
                    ...
                    if(total_time_vuv > time_cut && cut == true){ continue; }// continue to the next iteration without filling - don't bother filling it
                    ////////////////////////////////////////////////////////////////////
                ...
                if(num_VIS != 0 && config == 1) { //NOTE config == 1 is cathode foils configuration
                    ...
                    for(auto &y : transport_time_vis) { //looping through the transport_time_vis vector
                        ...
                        // 0.1 microseconds = 100 ns! 
                        if(total_time_vis > time_cut && cut == true){ continue; } // go onto the next interation - cut has been made
